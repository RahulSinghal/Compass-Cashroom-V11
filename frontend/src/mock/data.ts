...
const DEFAULT_USERS: User[] = [
  { id: 'U1', name: 'Sarah Mitchell',  email: 'operator@compass.com',   role: 'operator',   locationIds: ['APL-01'], active: true },
  { id: 'U2', name: 'David Clarke',    email: 'manager@compass.com',    role: 'controller',  locationIds: [], active: true },
  { id: 'U3', name: 'Lisa Chen',       email: 'controller@compass.com', role: 'controller',  locationIds: [], active: true },
  { id: 'U4', name: 'Robert Singh',    email: 'dgm@compass.com',        role: 'dgm',         locationIds: [], active: true },
  { id: 'U5', name: 'T. Admin',        email: 'admin@compass.com',      role: 'admin',       locationIds: [], active: true },
  { id: 'U6', name: 'Patricia Wells',  email: 'regional@compass.com',   role: 'regional-controller', locationIds: [], active: true },
]
// Merge defaults with stored — ensures demo accounts always exist even if localStorage has stale data
function loadUsersWithDefaults(): User[] {
  const stored = loadStored<User>('compass_users', [])
  if (!stored.length) return DEFAULT_USERS
  const merged = [...DEFAULT_USERS]
  stored.forEach(u => { if (!merged.find(m => m.email === u.email)) merged.push(u) })
  return merged
}
export const USERS: User[] = loadUsersWithDefaults()
